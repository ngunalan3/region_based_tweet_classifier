...
class tweet_classify:
	...
	def get_trends(self,sname):
		...
		api = tweepy.API(self.auth)
		#screen_name_collection = [ "CNN","CNNASIA24","CNN_NewsGroupEU"]
...
if __name__=="__main__":
	...
	#print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
	#tweet_arr = []
	#tweepy.Cursor(api.trends_place,1,exclude).append(tweet_arr):
    #print(tweet_arr)
	#		for status in tweepy.Cursor(api.trends_place,1,exclude).items(self.count):
			#for status in tweepy.Cursor(api.followers, screen_name="nand_gun").items(self.count):
	#			self.tweets.append(status.text)
	#			print(self.tweets.append(status.text))
	#	except Exception as e:
	#		print(e)
